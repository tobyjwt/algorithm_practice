...
// 这里使用的是最简单的、"传统"的二分查找法模板，使用"高级的"二分查找法模板在退出循环时候处理不方便
    while (left <= right) {
        ...
        let nums1LeftMax = i === 0 ? -Number.POSITIVE_INFINITY : nums1[i - 1];
        let nums1RightMin = i === m ? Number.POSITIVE_INFINITY : nums1[i];

        let nums2LeftMax = j === 0 ? -Number.POSITIVE_INFINITY : nums2[j - 1];
        let nums2RightMin = j ===n ? Number.POSITIVE_INFINITY : nums2[j];
