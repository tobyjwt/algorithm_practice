/*
有效 IP 地址 正好由四个整数（每个整数位于 0 到 255 之间组成，且不能含有前导 0），整数之间用 '.' 分隔。

例如："0.1.2.201" 和 "192.168.1.1" 是 有效 IP 地址，但是 "0.011.255.245"、"192.168.1.312" 和 "192.168@1.1" 是 无效 IP 地址。
给定一个只包含数字的字符串 s ，用以表示一个 IP 地址，返回所有可能的有效 IP 地址，这些地址可以通过在 s 中插入 '.' 来形成。你不能重新排序或删除 s 中的任何数字。你可以按 任何 顺序返回答案。


*/

function restoreIpAddresses(s: string): string[] {
    const n = s.length;
    if (n < 4) {
        return [];
    }
    const dp: string[][] = new Array(n).fill(0).map(() => new Array());
    dp[0] = [s[0], `${s[0]}.`];
    for (let i = 1; i < n; i++) {
        for (let j = 0; j < dp[i - 1].length; j++) {
            const splits = dp[i - 1][j].split('.');
            let cur: string = splits[splits.length - 1] + s[i];
            if(isLegal(cur)) {
                if (splits.length < 4 && i < n - 1) {
                    dp[i].push(dp[i - 1][j] + s[i]);
                    dp[i].push(dp[i - 1][j] + s[i] + '.');
                } else if (splits.length === 4) {
                    dp[i].push(dp[i - 1][j] + s[i]);
                }
            }
        }
    }
    return dp[n - 1];
};

function isLegal(s: string): boolean {
    if (s === '0') {
        return true;
    }
    if (s.indexOf('0') === 0) {
        return false;
    }
    return (Number(s) >= 0 && Number(s) <= 255);
}

// console.log(restoreIpAddresses('25525511135')); // ["255.255.11.135","255.255.111.35"]
// console.log(restoreIpAddresses('0000')); // ["0.0.0.0"]
// console.log(restoreIpAddresses('1111')); // ["1.1.1.1"]
console.log(restoreIpAddresses('010010')); // ["0.10.0.10","0.100.1.0"]
console.log(restoreIpAddresses('101023')); // ["1.0.10.23","1.0.102.3","10.1.0.23","10.10.2.3","101.0.2.3"]